...
extern int _fastEMAPeriod=3;
extern int _slowEMAPeriod=5;
extern int _signalPeriod=2;
...
extern double _lotSize=0.1;
extern int _takeProfitPips=100;
extern int _stopLossPips=100;
extern int _maxOrders=5;

extern bool _enableTrailing=true;
extern int _stopLossTrailInitialStartPips=50;
extern int _stopLossTrailPips=25;

extern int _profitWhenToSetNoLossStopPips=5;
extern int _profitOfNoLossStopPips=1;

extern int _atrAveragingPeriod=4;
extern double _atrThreshold=0.0012;

extern string _startPeriod1="00:05";
extern string _endPeriod1="23:55";
...
/////////////////////////////////////////////////////////////////////
// Return ints of BarsAgo for the following:
//  - UP  (UPWARD TRAJECTORY START) 
//  - CROSSUP
//  - DOWN (DOWNWARD TRAJECTORY START)
//  - CROSSDOWN
//  - PEAK (PEAKED)
//  - VALLEY (CAME OUT OF VALLEY)
////////////////////////////////////////////////////////////////////
struct indicatorInfo
   {
   int up; //count of bars since last start of up trajectory
   int down;
   int crossup; //count of bars since last crossup vs. either threshold or signal
   int crossdown;
   int peak;
   int valley; // turned valley x bars ago
   int flat; // turned flat x bars ago
   // where are we on the sine curve y=sin x
   // Starting from 0 going up to pi/6, pi/3, pi/2 (peak)
   // Coming down from peak 2/3pi, 5/6pi, pi
   // Going down from 0 - 7/6pi, 4/3pi, 3/2pi (valley)
   // coming up 5/3pi, 11/6pi, 2pi and the cycle starts again setting pi to 0
   int sinx; //http://www.nabla.hr/TrigF32.gif
   }
...
bool checkATR(int orderType){
   double atr;
   atr = iATR(NULL,0,_atrAveragingPeriod,0);
   if (atr > _atrThreshold) {
      return (true); }
   else {
      return (false);
   }
...
bool CheckStoch(int orderType){   
   ...
   // TODO fix the unnecessary looping here
...
int NeedOpenType(){
/*
 * This function decides if we want to open a Long position or a Short position
 * Algo:
 *    - If ATR is low, don't trade
 *    - If ATR is higher, then Stoch takes precedence, when it's relatively calm, then MACD
 *    - 
 */
   ...
   // MACD CROSS JUST HAPPENED and MACD HAS BEEN LOW FOR A WHILE
